...
select Product,count(Product) from bank_inventory_pricing 
where Month=1 
group by Product
...
select Product from bank_inventory_pricing 
where (Estimated_sale_price is null or purchase_cost> coalesce(Estimated_sale_price))
group by Product
having count(Product)>1;
...
# Question 7:
# 7) print the average of Purchase_cost from the table Bank_inventory_pricing.#
#	If the purchase_cost has  no value given
#   then it’s value is equal to estimated_sale_price.
select  avg(coalesce(purchase_cost,estimated_sale_price)) from bank_inventory_pricing;
...
select Product,sum(purchase_cost) from bank_inventory_pricing
...
select Product, avg(purchase_cost),sum(purchase_cost) from bank_inventory_pricing
group by Product
...
select max(Estimated_sale_price),sum(quantity)from bank_inventory_pricing 
...
# Question 17:
# 17) Print product with an increase in  average of estimated_sale_price  by 15%
# when average product_cost is more than average estimated_sale_price
select * from bank_inventory_pricing;
select Product,avg(purchase_cost),(avg(Estimated_sale_price)*15/100)+avg(Estimated_sale_price),
avg(Estimated_sale_price) from bank_inventory_pricing
group by Product
...
##select avg(purchase_cost) from bank_inventory_pricing
##where Product = 'BusiCard'
##group by Product
##having max(Price) or max(Estimated_sale_price);

#select avg(purchase_cost),coalesce(purchase_cost,max(price) or max(Estimated_sale_price)) 
...
select Product, AVG(COALESCE(Estimated_sale_price, purchase_cost)) from bank_inventory_pricing
GROUP BY Product;


# Question 20:
# 20) Print products and their avg price on condition that products appeared in at least three different months.
select Product,avg(price) from bank_inventory_pricing
group by Product
